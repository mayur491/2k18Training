	...
	function view_projects(projectId, userId)
	{
		$.getJSON('http://localhost:8080/M_d_ProjectManagementSystem/getEmployeeTasks.do?userId=' + userId + '&projectId=' + projectId, 
            ...
            for (var i = 0; i < data.length; i++){
            	...
            	space += '<td>'
            	space += '<a class = "css" href="setTaskStatus.do?id='+data[i][4]+'&status='+data[i][3]+'">' + data[i][3] + '</a>'
            	space += '</td>'
            	space += '</tr>'
            }
            space += '</table>'
			...
			<table>
				...
				<c:forEach items="${requestScope.projects}" var="projects">
					...
					<tr>
						<td><%= i %> <% i++; %></td>
						<td>${projects.name}</td>
						<td><button id ="${projects.id}" class="css" onclick='view_projects(this.id, "${userId}")'>${projects.status}</button></td>
					</tr>
